...
1. 다운로드한 ttf 파일에 마우스 오른쪽 버튼 → **"모든 사용자용으로 설치"**
   (메뉴에 없으면 더블클릭 → 왼쪽 위 [설치] 버튼)
2. 메모장을 열어 이 서체("BMHANNAPro Hangul11172")를 선택하고 `쫒` 입력 → 글자가 보이면 성공
3. 안 보이면 PC를 한 번 재부팅하세요

### macOS

1. 다운로드한 ttf 파일 더블클릭 → [설치] 클릭
2. 사용하던 앱(메모 등)을 완전히 종료(Cmd+Q) 후 다시 열기
3. 서체 목록에서 "BMHANNAPro Hangul11172"를 선택하고 `쫒` 입력 → 글자가 보이면 성공

### 확인

메모장/메모 앱에서 이 서체로 `쫒기다, 옷을 찟다` 를 입력해보세요.
모든 글자가 보이면 정상입니다.

## 자주 묻는 질문

**원래 한나체에서는 왜 글자가 안 보였나요?**
